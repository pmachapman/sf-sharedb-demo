...
import { hasStringProp, isMessageSendingOp, tryParseJSON } from "./util";
 ...
 * - This differs from the Scripture Forge implementation as it has no preventOpAcknowledgement support.
 */
export class ShareDBWebsocketAdapter extends ReconnectingWebSocket {
  /**
   * The listeners to call before sending an op.
   */
  private beforeSendOpListeners: ((
    collection: string,
    docId: string
  ) => Promise<void>)[] = [];

  /**
   * Whether or not to prevent op submission.
   */
  preventOpSubmission: boolean = false;

  /**
   * Adds a listener that will be called before an op is sent to the server.
   * This allows the op to be stored in IndexedDB before being sent.
   * @param listener The listener
   */
  subscribeToBeforeSendOp(
    listener: (collection: string, docId: string) => Promise<void>
  ): void {
  ...
  async send(data: Message): Promise<void> {
    ...
    if (sendingOp && this.preventOpSubmission) {
      return;
    }
    if (
      sendingOp &&
      hasStringProp(message, "c") &&
      hasStringProp(message, "d")
    ) {
      await this.beforeSendOp(message["c"], message["d"]);
    }
    super.send(data);
  }
